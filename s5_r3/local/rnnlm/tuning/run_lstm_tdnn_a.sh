...
for f in $text_dir/train.txt $wordlist; do
  [ ! -f $f ] && \
...
if [ $stage -le 0 ]; then
  mkdir -p $text_dir
  gunzip -c db/TEDLIUM_release-3/LM/*.en.gz lm_texts/*.gz |  gawk '{print(toupper($0))}' | sed 's/ <\/s>//g' > $text_dir/train.txt
  # gunzip -c db/TEDLIUM_release-3/LM/*.en.gz | sed 's/ <\/s>//g' > $text_dir/train.txt
  # shuffle text from audio and lm
  cat $text_from_audio |  gawk '{print(toupper($0))}' | cut -d ' ' -f2- | cat $text_dir/train.txt |\

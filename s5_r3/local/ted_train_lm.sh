...
if [ $stage -le 0 ]; then
  ...
  # Download the librispeech Textcorpus into lm_texts
  wget -nc https://openslr.elda.org/resources/11/librispeech-lm-norm.txt.gz -P lm_texts/

  # Unzip TEDLIUM 6 data sources, remove </s>, makes every character lowercase, gzip the result. | gawk '{print(tolower($0))}' 
  gunzip -c db/TEDLIUM_release-3/LM/*.en.gz lm_texts/*.gz | sed 's/ <\/s>//g' | gawk '{print(toupper($0))}' | gzip -c  > ${dir}/data/text/train.txt.gz
  ...
  awk '{print $1}' data/local/dict_nosp/lexicon.txt | sed 's:([0-9])::g' | sort | uniq > ${dir}/data/wordlist
fi

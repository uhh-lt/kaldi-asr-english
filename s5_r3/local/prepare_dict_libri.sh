...
cmudict_plain=$dst_dir/cmudict-0.7b.plain
...
# combining both lexica tedlium and librispeech
g2p_dir=$dst_dir/g2p
mkdir -p $g2p_dir
tl_lexicon=data/tl_lexicon.txt
# create wordlist for tedlium
python3 local/create_wordlist_from_lexicon.py
# find oov in tedlium
python3 local/find_oov.py -c data/train.tedlium/text -w data/local/dict_nosp.tedlium/wordlist_autogen.txt -o data/oov_tedlium.txt
# find oov in librispeech
python3 local/find_oov.py -c data/train_clean_100.librispeech/text -w data/librispeech/lm/librispeech-vocab.txt -o data/oov_librispeech.txt
# combine both oovs and remove duplicates
sort data/oov_tedlium.txt data/oov_librispeech.txt | uniq > $g2p_dir/vocab_autogen
# combine the lexicons
sort data/local/dict_nosp.tedlium/lexicon.txt $cmudict_plain | uniq > $tl_lexicon
# remove all Token with "<"
sed -i '/</d' $tl_lexicon


if [ $stage -le 1 ]; then
  # check if we have Sequitur G2P is installed (doesnt work)
  # if [ ! -f  "$SEQUITUR" ]; then
  ...
  awk 'NR==FNR{a[$1] = 1; next} !($1 in a)' $tl_lexicon $vocab |\
...
if [ $stage -le 2 ]; then
  echo "Combining the tedlium and librispeech pronunciations with the autogenerated ones ..."
  awk 'NR==FNR{a[$1]=1; next} ($1 in a)' $vocab $tl_lexicon |\
